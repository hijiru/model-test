...
import{technicalindicators,past}from'../index.js'
...
export const wallet:number[] = [binanace_wallet]

export const positions:number[] = [,,,0]
export const data:number[] = []
export const order:string[] = []
export const q:number[] = [wallet[0]]
export const d :number[] = [0]
...
export async function simulated_trade(){
    ...
    // else if(Earn<=-5&&position==counter||positions[3]-Earn>=2&&Earn>5&&position==counter){
      ...
      if(gol>0){n+=1;Gain+=gol;}else{Loss+=gol}
      wallet[0]=close
      positions[3]=0
      q.push(wallet[0])
      if(Earn-positions[3]<drawdown){drawdown=Earn-positions[3]}
     ...
     process.nextTick(await game_over(`Forced Liquidation`) as unknown as Function)

     }else if(enough){
   
      process.nextTick(await game_over(`Insufficient balance`) as unknown as Function)

     }else if(times==end_times){
  
      process.nextTick(await game_over(`simulated trading is done`) as unknown as Function)

     }


    async function game_over(comment:string){
...
export   function average(l:number,a:number){
  // console.log(data)
  let x 
    if(data.length<l){
      data.unshift(a)
